...
{ 
  ...
  if ((x->fp = fopen(x->filename, "r"))==NULL) { 
    fprintf(stderr, "lex: cannot open %s\n", x->filename);
    exit(EXIT_FAILURE);
  }
  x->c = ' ';
  x->linenum = 1;
...
static int tmp_stack1744700218 = 1580;  
...
{ 
  ...
  else if (x->c=='%') {token_c_append(x); c_get(x); x->type = token_PERCENT;}  
  else if (x->c=='&') {token_c_append(x); c_get(x); x->type = token_AND;}  
  else if (x->c=='=') {
    token_c_append(x); c_get(x);
    if (x->c=='=') {token_c_append(x); c_get(x); x->type = token_EQEQ;}        
    else {x->type = token_EQ;}
  }
  else if (x->c=='<') {
    token_c_append(x); c_get(x);
    if (x->c=='=') {token_c_append(x); c_get(x); x->type = token_LE;}  
    else {x->type = token_LT;}
  }
  else if (x->c=='>') {
    token_c_append(x); c_get(x);
    if (x->c=='=') {token_c_append(x); c_get(x); x->type = token_GE;}  
    else {x->type = token_GT;}
  }
  else if (x->c=='!') {
    token_c_append(x); c_get(x);
    if (x->c=='=') {token_c_append(x); c_get(x); x->type = token_NE;}  
  }
  /* ここまで */  
