...
{  
  ...
  for(int i=0; i<(star_counter); i++) {code_append(c, opcode_LI, 0, 0);}
...
// ”;” を確認すればよい.
  ...
  parse_expression(c, x, gt, lt);
  if(x->type == token_SEMICOLON) {lex_get(x);}
  else {syntax_error(x, "'m;' expected");}
...
//  ISP (STACK FRAME RESERVE)
//  (第 1 引数の 式 のコード)
//  (第 2 引数の 式 のコード)
//  …
//  (第 n 引数の 式 のコード)
//  ISP -(STACK FRAME RESERVE + n)
//  CALL (関数の開始番地)
  else if(id_isfunc(x->token, gt, lt)) {
    code_append(c, opcode_ISP, STACK_FRAME_RESERVE, 0);

    //  関数名がgt表の何番目か
    int func_index = tab_itab_find(gt, x->token);
    lex_get(x);
    if(x->type==token_LPAREN) {lex_get(x);}
    else {syntax_error(x, "'(' expected");}
    int buffo=0;
    ...
    if(func_index!=itab_FAIL) {code_append(c, opcode_CALL, gt->itab[func_index].address, 0);}
    else {printf("this function not defined.");}
    lex_get(x);
  }
